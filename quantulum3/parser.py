...
def clean_surface(surface, span):
    ...
    no_start = ['and', ' ']
    no_end = [' '] + [' {}'.format(misc) for misc in reg.MISCNUM]
    ...
    if split[0] in reg.MISCNUM and len(split) > 1 and split[1] in \
            reg.UNITS + reg.TENS:
...
def extract_spellout_values(text):
    ...
    values = []
    for item in reg.REG_TXT.finditer(text):
        ...
        for word in surface.split():
            try:
                scale, increment = 1, float(
                    re.sub(r'(-$|[%s])' % reg.GROUPING, '', word.lower()))
            except ValueError:
                scale, increment = reg.NUMWORDS[word.lower()]
...
def get_values(item):
    ...
    def callback(pattern):
        return ' %s' % (reg.UNI_FRAC[pattern.group(0)])

    fracs = r'|'.join(reg.UNI_FRAC)

    value = item.group('value')
    # Remove grouping operators
    value = re.sub(r'(?<=\d)[%s](?=\d{3})' % reg.GROUPING, '', value)
    # Replace unusual exponents by e (including e)
    value = re.sub(r'(?<=\d)(%s)(e|E|10)\^?' % reg.MULTIPLIERS, 'e', value)
